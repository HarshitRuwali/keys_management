...
#         collection = db["keys"]
#         res = collection.find_one({"_id": ObjectId(key)})
#         if res["use_time"] + timedelta(0, 60) < datetime.now():
#             update_res = collection.find_one_and_update(
#                 {"_id": ObjectId(key)},
#                 {
#                     "$set": {"free_time": datetime.now(), "available": True}
#                 }
#             )
#             KEYS_IN_USE.remove(str(key))
#             print(update_res)
...
def _is_key_in_use():
    while thread_event.is_set():
        ...
        for key in KEYS_IN_USE:
            ...
            collection = db["keys"]
            res = collection.find_one({"_id": ObjectId(key)})
            if res["use_time"] + timedelta(0, 5) < datetime.now():
                update_res = collection.find_one_and_update(
                    {"_id": ObjectId(key)},
                    {
                        "$set": {
        ...
        thread_event.clear()
        print("stopped the thread as the all the keys are now inactive")
